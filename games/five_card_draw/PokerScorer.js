class PokerScorer {
    ...
    /*
        ...
        Reduced repetition and unnecessary additional static methods for other checks
        that used the same processes simply with different outcomes. Still feels slightly
        repetitive?
    */
    static scoreHand(hand) {
        const pips = hand.map((card) => card.value);
        ...
        if (PokerScorer.isStraightFlush(uniquePips, uniqueSuits)) {
            return uniquePips.every((pip) => pip > 9)
    ...
    static isStraight(pips) {
        if (pips.length !== 5) return false;

        const sortedPips = pips.toSorted((a, b) => a - b);
        ...
        const isAceThroughFive = [2, 3, 4, 5, 14].every((number, i) => number === sortedPips[i]);

        return isFiveConsecutivePips || isAceThroughFive;
    }

    static isStraightFlush(pips, uniqueSuits) {
