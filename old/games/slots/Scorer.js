class Scorer {
    ...
    static scoreWinningLines(reels) {
        /* 
            For scalability, this can be refactored to assign an object of win lines based on a separate file with
            different configurations (e.g. simpler/more complex patterns, different sized/number of reels) by
        ...
        */
        ...
        Scorer.matches.length = 0;

        const MINIMUM_MATCH_LENGTH = 3;
        let score = 0;

        for (const [patternNumber, reelPositions] of Object.entries(winLines)) {
            const [maxConsecutives, matchEmoji] = Scorer.countConsecutiveRepeats(reelPositions);

            const matchesFromStartOrEnd =
                matchEmoji === reelPositions[0] || matchEmoji === reelPositions.at(-1);
            ...
            if (maxConsecutives >= MINIMUM_MATCH_LENGTH && matchesFromStartOrEnd) {
                ...
                Scorer.matches.push([Array(maxConsecutives).fill(matchEmoji), patternNumber]);
            }
        }
    ...
    */
    static countConsecutiveRepeats(line) {
