...
export function getInitialSlotsEmbed(user, reels, matches, score, isLuckyStar) {
    ...
    matches.forEach(([emojis, patternNumber]) => {
...
function addScoreFieldsToEmbed(embed, score, isLuckyStar) {
    ...
    embed.addFields({
        ...
        value: isLuckyStar
            ? `Your score of ${score / 2} points has been doubled to **${score}** points${extraMessage}`
            : `You scored **${score}** points${extraMessage}`,
    });
}
export function addBetResultsToEmbed(embed, bet, earnings, userGoldString) {
    embed.addFields({ name: 'Bet placed', value: `${bet.toLocaleString('en-US')}${gold}`, inline: true }, {
