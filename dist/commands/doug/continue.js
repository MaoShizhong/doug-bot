...
const command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) => option.setName('id').setDescription('ID of message to continue from').setRequired(true))
    ...
    async execute(interaction) {
        ...
        await interaction.editReply(`Continuing from: https://discord.com/channels/${interaction.guild?.id}/${interaction.channel?.id}/${msgID}\n${optionalPrompt}\n${response.content.slice(0, 1850 - (prompt ? prompt.length : 0))}`);
    },
};
