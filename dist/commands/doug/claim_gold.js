...
const command = {
    ...
    async execute(interaction) {
        ...
        const UserInServer = User[interaction.guildId];
        ...
        if (user.canClaimGold) {
            ...
            await Promise.all([
                user.save(),
                interaction.reply({
                    content: `+${GOLD_CLAIM_AMOUNT}${gold}! ` +
                        `You currently have ${user.goldString}${gold}\n` +
                        `You may claim ${GOLD_CLAIM_AMOUNT}${gold}for free up to once every hour. Your next claim will be available in ${Math.round(GOLD_CLAIM_COOLDOWN_MS / 60000)} minutes.`,
            ...
            await interaction.reply({
                content: `Sorry, you have already claimed ${gold} within the last hour! You may claim again in ${remainingTimeInMins} ${remainingTimeInMins === 1 ? 'minute' : 'minutes'}.`,
                allowedMentions: { repliedUser: false },
            });
        }
    },
